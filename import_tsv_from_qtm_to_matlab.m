...
% Events
eventsArray = textscan(fileID, '%s', 15, 'Delimiter', delimiter, ...
    'EmptyValue' ,NaN,'HeaderLines', 0, 'ReturnOnError', false, 'EndOfLine', '\r\n');
...
X=reshape(dataArray{1},nchannels*3+2,[])';
T=X(:,1:2);
X=reshape(X(:,3:end),[],3,nchannels);
clear dataArray


% It's easier to see the NaNs when dealing with missing frames. QTM can
% export missing frames either as NaNs or zeros.
X(X==0) = nan;
